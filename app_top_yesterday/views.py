...
# load data
def load_data_topYesterday():
    # read df
    df_topYesterday = pd.read_csv(
        'app_top_yesterday\dataset\popular-WORK_OF_ART-of-yesterday.csv')
    # refresh data
    global data # make data global. It can be used everywhere.
    data = {}
    for idx, row in df_topYesterday.iterrows():
        data[row['category']] = eval(row['top_keys'])


# Load data first when starting server.
load_data_topYesterday()


def home(request):
    return render(request, 'app_top_yesterday/home.html')

# csrf_exempt is used for POST
# 單獨指定這一支程式忽略csrf驗證
@csrf_exempt
def api_get_topYesterday(request):
    ...
    chart_data, wf_pairs = get_category_topYesterday(cate, topk)
...
def get_category_topYesterday(cate, topk):
